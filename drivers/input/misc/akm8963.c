...
#define VENDOR_NAME             	"AsahiKASEI"
#define SENSOR_NAME 			    "AK8963C"
...
static int akm8963c_power_ctl(struct akm8963_data *pdata, bool on)
{
	int ret = 0;

	if (!on && pdata->power_enabled) {
		ret = regulator_disable(pdata->vdd);
		if (ret) {
			dev_err(&pdata->i2c->dev,
				"Regulator vdd disable failed ret=%d\n", ret);
			return ret;
		}

		ret = regulator_disable(pdata->vio);
		if (ret) {
			dev_err(&pdata->i2c->dev,
		...
		dev_dbg(&pdata->i2c->dev, "stk3x1x_power_ctl on=%d\n",
				on);
	} else if (on && !pdata->power_enabled) {
		...
		ret = regulator_enable(pdata->vio);
		if (ret) {
			dev_err(&pdata->i2c->dev,
		...
		dev_warn(&pdata->i2c->dev,
				"Power on=%d. enabled=%d\n",
				on, pdata->power_enabled);
	}

	return ret;
}

static int akm8963c_power_init(struct akm8963_data *pdata, bool on)
{
	int ret;

	if (!on) {
		if (regulator_count_voltages(pdata->vdd) > 0)
			regulator_set_voltage(pdata->vdd,
					0, STK3X1X_VDD_MAX_UV);

		regulator_put(pdata->vdd);

		if (regulator_count_voltages(pdata->vio) > 0)
			regulator_set_voltage(pdata->vio,
					0, STK3X1X_VIO_MAX_UV);

		regulator_put(pdata->vio);
	} else {
		pdata->vdd = regulator_get(&pdata->i2c->dev, "vdd");
		if (IS_ERR(pdata->vdd)) {
		...
		if (regulator_count_voltages(pdata->vdd) > 0) {
			ret = regulator_set_voltage(pdata->vdd,
					STK3X1X_VDD_MIN_UV,
					STK3X1X_VDD_MAX_UV);
			if (ret) {
				dev_err(&pdata->i2c->dev,
		...
		pdata->vio = regulator_get(&pdata->i2c->dev, "vio");
		if (IS_ERR(pdata->vio)) {
...
reg_vdd_set:
	if (regulator_count_voltages(pdata->vdd) > 0)
		regulator_set_voltage(pdata->vdd, 0, STK3X1X_VDD_MAX_UV);
reg_vdd_put:
	regulator_put(pdata->vdd);
	return ret;
}
	...
	int *rbuf)
{
	...
	AKM_DATA(&akm->i2c->dev, " AKM8963 %s: flag =0x%X\n", __func__,
		rbuf[0]);
	AKM_DATA(&akm->i2c->dev, "  Acceleration[LSB]: %6d,%6d,%6d stat=%d\n",
		rbuf[1], rbuf[2], rbuf[3], rbuf[4]);
	AKM_DATA(&akm->i2c->dev, "  Geomagnetism[LSB]: %6d,%6d,%6d stat=%d\n",
		rbuf[5], rbuf[6], rbuf[7], rbuf[8]);
	AKM_DATA(&akm->i2c->dev, "  Orientation[YPR] : %6d,%6d,%6d\n",
		rbuf[9], rbuf[10], rbuf[11]);
...
static ssize_t ak8963_info_show(struct device *dev, struct device_attribute *attr, char *buf){
...
static struct device_attribute akm8963_attributes[] = {
	...
	__ATTR(info,  0440, ak8963_info_show,  NULL),
#ifdef AKM8963_DEBUG_IF
...
static int akm8963c_parse_dt(struct device *dev, struct akm8963_platform_data *pdata)
{
	int rc;
	struct device_node *np = dev->of_node;
	u32 temp_val;

	pdata->gpio_DRDY = of_get_named_gpio_flags(np, "akm,gpio_DRDY",
				0, &pdata->int_flags);
	if (pdata->gpio_DRDY < 0) {
		dev_err(dev, "Unable to read gpio_DRDY\n");
		return pdata->gpio_DRDY ;
	}
...
int akm8963_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		err = akm8963c_parse_dt(&client->dev, pdata);
		if (err)
			return err;
	} else {
		pdata->layout = 0;
	...
	if (gpio_is_valid(pdata->gpio_DRDY)) {
		err = gpio_request(pdata->gpio_DRDY , "akm8963c_irq_gpio");
		if (err) {
	...
	err = akm8963c_power_init(s_akm, true);
	if (err)
		goto exit2;

	err = akm8963c_power_ctl(s_akm, true);
	if (err)
		goto exit2;
...
static const struct of_device_id akm8963c_of_match[] = {
...
static struct i2c_driver akm8963_driver = {
	...
	.driver = {
		...
		.owner = THIS_MODULE,
		.of_match_table = akm8963c_of_match,
	},
};
