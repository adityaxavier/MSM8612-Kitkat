...
//@wb add 2014.07.15 
static long  gtxx_id = 0; //for selecting tp config group
//@wb add 2014.07.15 
...
//	unsigned char *config_data;
	...
	else if (cfg_info_len[2]) /*Modify start: LCD ID check when LCD IC is the same */
	{
		sensor_id = gpio_get_value(76);
		dev_info(&client->dev, "Sensor ID selected: %d\n", sensor_id);
	}

	//@wb add start 2014.07.15
	#ifdef CONFIG_PRODUCT_CALD03
		sensor_id = 0;
	if (gtxx_id == GT660_ID)
	{
		sensor_id =1;
	...
	#endif
	//@wb add end 2014.07.15
	
	
	cfg_len = cfg_info_len[sensor_id];

	printk("CTP_CONFIG_GROUP:%d used, config length: %d", sensor_id + 1, cfg_len);
	
	ts->pdata->config_data_len[sensor_id] = cfg_info_len[sensor_id];
	...
	memcpy(&config_data[GTP_ADDR_LENGTH], send_cfg_buf[sensor_id], cfg_len);
...
//	struct goodix_ts_data *ts = i2c_get_clientdata(client);
	...
	//@wb add start 2014.07.15
	if (kstrtol(product_id,0,&gtxx_id) < 0) //just geting product name
	{
		dev_info(&client->dev,"Can't convert!\n");
		gtxx_id = 0;
	}
	dev_info(&client->dev,"gt_product name:%ld\n", gtxx_id);
	//@wb add end 2014.07.15
...
//	char prop_name[PROP_NAME_SIZE];
	...
	*/
	...
	printk("%d config data read from device tree.\n", read_cfg_num);
#endif
	return 0;
}
...
//delete bagin linxiangfa@20140331, for tp touch enent report maybe Conflict.
/*
	for (i = 0; i < GTP_MAX_TOUCH; i++)
		gtp_touch_up(ts, i);
