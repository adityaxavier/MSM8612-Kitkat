...
///@Hxin add for avoid AF motor current leak start
void camera_af_software_powerdown(struct i2c_client *client)
{
	...
	CDBG("camera_af_software_powerdown IN...\n");
	/* send software powerdown cmd to AF motor, avoid current leak */	
	rc = i2c_transfer(client->adapter, msg, 1);
	if (rc < 0)
		pr_err("AF software powerdown faild\n");
}
EXPORT_SYMBOL(camera_af_software_powerdown);
///@Hxin add for avoid AF motor current leak end
